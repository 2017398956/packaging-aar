...
class VariantProcessor {
    ...
    VariantProcessor(Project project, LibraryVariantImpl variant) {
        mProject = project
        mVariant = variant
    ...
    private void transformRClasses(RClassesTransform transform, TaskProvider transformTask, TaskProvider bundleTask, TaskProvider reBundleTask) {
        transform.putTargetPackage(mVariant.name, FatAarPlugin.customApplicationId == null ? mVariant.applicationId : FatAarPlugin.customApplicationId)
        transformTask.configure {
            doFirst {
