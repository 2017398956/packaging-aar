...
class FatAarPlugin implements Plugin<Project> {
    ...
    public static String customApplicationId
    ...
    private void doAfterEvaluate() {
        ...
        project.android.libraryVariants.all { variant ->
            try{
                customApplicationId = ((LibraryVariantImpl) variant).applicationId
            }catch(Exception e){
                // e.printStackTrace()
            }
            if (null == customApplicationId || customApplicationId.length() == 0){
                if (null == project.fataar.applicationId){
                    throw new Exception('You must add "applicationId = your library package name" in fataar{}')
                }
                customApplicationId = project.fataar.applicationId
            }
    ...
    private Collection<ResolvedArtifact> dealUnResolveArtifacts(Configuration configuration, LibraryVariantImpl variant, Collection<ResolvedArtifact> artifacts) {
