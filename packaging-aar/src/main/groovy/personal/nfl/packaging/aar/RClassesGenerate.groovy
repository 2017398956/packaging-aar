...
class RClassesGenerate {
    ...
    private final LibraryVariantImpl mVariant

    private VersionAdapter mVersionAdapter
    private final Collection<AndroidArchiveLibrary> mLibraries

    RClassesGenerate(Project project, LibraryVariantImpl variant, Collection<AndroidArchiveLibrary> libraries) {
        mProject = project
        mVariant = variant
    ...
    private def createRFile(AndroidArchiveLibrary library, def rFolder, ConfigObject symbolsMap) {
        def libPackageName = FatAarPlugin.customApplicationId == null ? mVariant.applicationId : FatAarPlugin.customApplicationId
    ...
    private TaskProvider configureRJarTask(final File fromDir, final File desFile, final TaskProvider reBundleAarTask) {
        ...
        TaskProvider task = mProject.getTasks().register(taskName, Jar) {
            ...
            if (FatUtils.compareVersion(mProject.gradle.gradleVersion, "5.1") >= 0) {
                it.getArchiveFileName().set("${FatAarPlugin.customApplicationId == null ? mVariant.applicationId : FatAarPlugin.customApplicationId}-r-classes.jar")
                it.getDestinationDirectory().set(desFile)
            } else {
                it.archiveName = "${FatAarPlugin.customApplicationId == null ? mVariant.applicationId : FatAarPlugin.customApplicationId}-r-classes.jar"
